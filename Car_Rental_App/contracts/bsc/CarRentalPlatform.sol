...
struct User {
  ...
  string name;
  string lastName;
  uint rentedCarId; //eğer id 0 gelirse user araç kiralamamış
  uint balance;
  uint debt;
  uint start; // saniye cinsinden olacak dakikaya çevireceğiz
}
...
function addCar(string calldata name, string calldata url,uint rent ,uint sale )external onlyOwner{
  _counter.increment(); //sayaç arttırıp araba id sini tanımlayacağız.
...
//editCarMetadata #onlyOnwner #existingCar
...
//checkOut #existingUser #isCarAvailable #userHasNotRentedACar #userHasNotDebt
...
//makePayment #existingUser #existingDebt #suffiecentBalance
...
function getContractBalance()external view onlyOwner returns(uint) {
  return address(this).balance; // this bu kontrat'ı temsil ediyor.
}
